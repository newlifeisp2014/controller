...
import {
  show_led_control_modal as _show_led_modal,
  on_led_modal_hidden,
  ledColorChanged,
  ...
  setPlayerLed,
  applyLedSettings
} from './modals/led-control-modal.js';
...
function update_battery_status({ charge_level, cable_connected, is_charging, is_error, bat_txt, changed }) {
  if(changed) {
    $("#d-bat").html(bat_txt);
    _update_battery_card(charge_level, cable_connected, is_charging, is_error);
  }
}

function _update_battery_card(level, cableConnected, isCharging, isError) {
  const pct = typeof level === 'number' ? Math.round(level * 100) : null;
  const fill = document.getElementById('bat-fill');
  const pctEl = document.getElementById('bat-pct');
  const statusEl = document.getElementById('bat-status-text');
  ...
  if (pctEl) pctEl.textContent = pct !== null ? displayPct + '%' : '--';

  // Color the fill bar
  fill.classList.remove('warning', 'danger', 'charging');
  if (isCharging) {
  ...
  if (iconEl) {
    ...
    if (isError) icon = 'fa-battery-slash';
    else if (isCharging) icon = 'fa-bolt';
    else if (pct !== null && pct <= 10) icon = 'fa-battery-empty';
    else if (pct !== null && pct <= 30) icon = 'fa-battery-quarter';
    else if (pct !== null && pct <= 60) icon = 'fa-battery-half';
    else if (pct !== null && pct <= 85) icon = 'fa-battery-three-quarters';
    iconEl.innerHTML = `<i class="fas ${icon}"></i>`;
  }

  // Status text
  if (statusEl) {
    if (isError) statusEl.textContent = 'خطأ في البطارية';
    else if (isCharging) statusEl.textContent = '⚡ جارٍ الشحن...';
    else if (cableConnected) statusEl.textContent = 'USB متصل';
    else statusEl.textContent = 'لاسلكي';
  }

  nlLog(pct !== null ? `البطارية: ${displayPct}%${isCharging ? ' (يشحن)' : ''}` : 'البطارية: غير معروفة', 'debug');
}
  ...
  switch (current_active_tab) {
    case 'controller-tab': // Main controller tab
      ...
      if(isFinetuneVisible()) {
        ...
        // Push to live analog chart
        if (changes.sticks) {
          const { left, right } = changes.sticks;
          _pushChartData(left?.x ?? 0, left?.y ?? 0, right?.x ?? 0, right?.y ?? 0);
        }
      }
      break;

    case 'features-tab':
      // Update gyro/accel display if active
      if (gyro || accel) {
        _update_gyro_display(gyro, accel);
      }
      // Update touchpad coordinates display
      _update_touchpad_display(touchPoints);
      break;
...
const CHART_HISTORY = 120; // points
const _chartData = { lx: [], ly: [], rx: [], ry: [] };
const _chartColors = { lx: '#6387ff', ly: '#a78bfa', rx: '#34d399', ry: '#fbbf24' };

function _initAnalogChart() {
  for (const k of Object.keys(_chartData)) _chartData[k] = new Array(CHART_HISTORY).fill(0);
}

function _pushChartData(lx, ly, rx, ry) {
  for (const [k, v] of [['lx', lx], ['ly', ly], ['rx', rx], ['ry', ry]]) {
    _chartData[k].push(v);
    if (_chartData[k].length > CHART_HISTORY) _chartData[k].shift();
  }
  _drawAnalogChart();
}

function _drawAnalogChart() {
  const canvas = document.getElementById('analog-live-chart');
  if (!canvas) return;
  ...
  // Background grid
  ctx.strokeStyle = 'rgba(99,135,255,0.06)';
  ctx.lineWidth = 1;
  for (let i = 0; i <= 4; i++) {
    const y = (H / 4) * i;
  ...
  // Center line
  ctx.strokeStyle = 'rgba(99,135,255,0.12)';
  ctx.beginPath(); ctx.moveTo(0, H/2); ctx.lineTo(W, H/2); ctx.stroke();

  // Draw lines
  const step = W / (CHART_HISTORY - 1);
  for (const [key, color] of Object.entries(_chartColors)) {
    const data = _chartData[key];
    ctx.beginPath();
...
const GYRO_HIST = 80;
const _gyroData = { gx: [], gy: [], gz: [], ax: [], ay: [], az: [] };
let _gyroActive = false;

function _initSensorCharts() {
  for (const k of Object.keys(_gyroData)) _gyroData[k] = new Array(GYRO_HIST).fill(0);
}
_initSensorCharts();

function toggleGyroTest() {
  _gyroActive = !_gyroActive;
  const btn = document.getElementById('gyro-toggle-btn');
  if (btn) {
    btn.innerHTML = _gyroActive
      ? '<i class="fas fa-stop me-1"></i>إيقاف'
      : '<i class="fas fa-play me-1"></i>تشغيل';
    btn.className = _gyroActive
      ? 'btn btn-danger btn-sm'
      : 'btn btn-outline-secondary btn-sm';
  }
  if (_gyroActive) nlLog('تم تفعيل اختبار الجيروسكوب', 'info');
  else nlLog('تم إيقاف اختبار الجيروسكوب', 'info');
}

function _drawSensorChart(canvasId, data1, data2, data3, c1, c2, c3) {
  ...
  const step = W / (GYRO_HIST - 1);
  const maxAbs = data => Math.max(...data.map(Math.abs), 1);

  for (const [data, color] of [[data1,c1],[data2,c2],[data3,c3]]) {
    const scale = maxAbs(data);
    ctx.beginPath();
    ctx.strokeStyle = color;
    ctx.lineWidth = 1.5;
    data.forEach((v, i) => {
      const x = i * step;
      const y = H/2 - (v / scale) * (H/2 - 4);
      i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
    });
    ctx.stroke();
  }
...
function _update_gyro_display(gyro, accel) {
  if (!_gyroActive) return;
  // gyro: {x, y, z}  accel: {x, y, z}
  const gx = gyro?.x ?? 0, gy = gyro?.y ?? 0, gz = gyro?.z ?? 0;
  const ax = accel?.x ?? 0, ay = accel?.y ?? 0, az = accel?.z ?? 0;

  for (const [k, v] of [['gx',gx],['gy',gy],['gz',gz],['ax',ax],['ay',ay],['az',az]]) {
    _gyroData[k].push(v);
    if (_gyroData[k].length > GYRO_HIST) _gyroData[k].shift();
  }

  const fmt = n => (n >= 0 ? '+' : '') + n.toFixed(2);
  const setEl = (id, v) => { const e = document.getElementById(id); if(e) e.textContent = fmt(v); };
  setEl('gyro-x-val', gx); setEl('gyro-y-val', gy); setEl('gyro-z-val', gz);
  setEl('accel-x-val', ax); setEl('accel-y-val', ay); setEl('accel-z-val', az);

  _drawSensorChart('gyro-chart',  _gyroData.gx, _gyroData.gy, _gyroData.gz, '#6387ff','#34d399','#fbbf24');
  _drawSensorChart('accel-chart', _gyroData.ax, _gyroData.ay, _gyroData.az, '#f87171','#a78bfa','#38bdf8');
}
...
function nlLog(msg, type = 'action') {
  const now = new Date();
  ...
  // Also forward to actual console
  console.log(`[${type}] ${msg}`);
}

function _appendLogEntry(entry) {
  const console_el = document.getElementById('log-console');
  const empty_el = document.getElementById('log-empty');
  if (!console_el) return;

  if (_logFilter !== 'all' && _logFilter !== entry.type) return;
...
function filterLogs() {
  ...
  if (filtered.length === 0) {
    if (empty_el) empty_el.style.display = '';
  } else {
    if (empty_el) empty_el.style.display = 'none';
    filtered.forEach(e => _appendLogEntry(e));
  }
}
...
function exportLog() {
  const text = _logEntries.map(e => `[${e.time}] [${e.type}] ${e.msg}`).join('\n');
  const blob = new Blob([text], { type: 'text/plain' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `newlife-log-${new Date().toISOString().slice(0,19).replace(/:/g,'-')}.txt`;
  a.click();
  URL.revokeObjectURL(url);
  nlLog('تم تصدير السجل', 'success');
}
...
function exportSettingsJSON() {
  try {
    const keys = Object.keys(localStorage).filter(k =>
      k.startsWith('changes_') || k.startsWith('finetune') ||
      k === 'centerCalibrationMethod' || k === 'rangeCalibrationMethod' ||
      k === 'quickTestSkippedTests'
    );
    const data = {};
    ...
    data._tool_version = 'NEWLIFE Controller Tool';

    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `newlife-settings-${new Date().toISOString().slice(0,10)}.json`;
    a.click();
    URL.revokeObjectURL(url);
    nlLog('تم تصدير الإعدادات كـ JSON', 'success');
    successAlert('تم تصدير الإعدادات بنجاح ✓');
  } catch (err) {
    nlLog('فشل تصدير الإعدادات: ' + err.message, 'error');
    errorAlert('فشل التصدير: ' + err.message);
  }
}
...
function importSettingsJSON(event) {
  ...
  reader.onload = (e) => {
    try {
      ...
      if (!data._tool_version) {
        warningAlert('الملف لا يبدو أنه إعدادات NEWLIFE صحيحة');
        return;
      }
      let imported = 0;
      ...
      nlLog(`تم استيراد ${imported} إعداد من الملف`, 'success');
      successAlert(`✓ تم الاستيراد — ${imported} إعداد`);
    } catch (err) {
      nlLog('فشل استيراد الإعدادات: ' + err.message, 'error');
      errorAlert('الملف غير صالح: ' + err.message);
    }
    event.target.value = '';
  };
...
async function startMicTest() {
  try {
    ...
    document.getElementById('mic-start-btn')?.classList.add('d-none');
    document.getElementById('mic-stop-btn')?.classList.remove('d-none');
    nlLog('تم تفعيل اختبار الميكروفون', 'info');
    ...
    const tick = () => {
      ...
      const lbl = document.getElementById('mic-level-pct');
    ...
    nlLog('فشل الوصول للميكروفون: ' + err.message, 'error');
    errorAlert('لا يمكن الوصول للميكروفون: ' + err.message);
  }
}
...
function stopMicTest() {
  ...
  _micAnalyser = null;
  document.getElementById('mic-start-btn')?.classList.remove('d-none');
  document.getElementById('mic-stop-btn')?.classList.add('d-none');
  const bar = document.getElementById('mic-level-bar');
  const lbl = document.getElementById('mic-level-pct');
  if (bar) bar.style.width = '0%';
  if (lbl) lbl.textContent = '0%';
  nlLog('تم إيقاف اختبار الميكروفون', 'info');
}
...
function _update_touchpad_display(touchPoints) {
  const pts = Array.isArray(touchPoints) ? touchPoints : [];
  const p1 = pts[0];
  const p2 = pts[1];

  const setCoord = (xId, yId, pt) => {
    const xe = document.getElementById(xId);
    const ye = document.getElementById(yId);
    if (!xe || !ye) return;
    if (pt?.active) {
      xe.textContent = pt.x;
      ye.textContent = pt.y;
  ...
  setCoord('tp-x1', 'tp-y1', p1);
  setCoord('tp-x2', 'tp-y2', p2);
}
...
function confirmFactoryReset() {
  show_popup(
    `<div class="text-center">
      <i class="fas fa-triangle-exclamation fa-3x mb-3" style="color:var(--nl-accent-red)"></i>
      <h5 style="color:var(--nl-text)">إعادة ضبط المصنع</h5>
      <p style="color:var(--nl-text-muted)">
        هذه العملية ستمحو <b>جميع إعدادات المعايرة المخزنة</b> في الدراعة.
        لا يمكن التراجع عن هذا الإجراء.
      </p>
      <div class="d-flex gap-2 justify-content-center mt-3">
        <button class="btn btn-danger" onclick="executeFactoryReset()">
          <i class="fas fa-trash me-1"></i>تأكيد إعادة الضبط
        </button>
...
async function executeFactoryReset() {
  if (!controller?.isConnected()) {
    errorAlert('لا يوجد جهاز متصل');
    return;
  }
  try {
    nlLog('بدء إعادة ضبط المصنع...', 'warn');
    // Close confirmation modal
    bootstrap.Modal.getInstance('#popupModal')?.hide();
    // Try factory reset via controller
    if (typeof controller?.currentController?.factoryReset === 'function') {
      await controller.currentController.factoryReset();
    } else {
      // Fallback: reboot (best we can do without explicit factory reset support)
      await controller.reset();
    }
    nlLog('تمت إعادة ضبط المصنع بنجاح', 'success');
    successAlert('✓ تمت إعادة ضبط المصنع');
  } catch (err) {
    nlLog('فشل إعادة ضبط المصنع: ' + err.message, 'error');
    errorAlert('فشل: ' + err.message);
  }
}
...
function show_led_control_modal() {
  if (!controller?.isConnected()) {
    infoAlert('قم بتوصيل الدراعة أولاً');
    return;
  }
  _show_led_modal(controller);
  nlLog('فتح نافذة التحكم بالإضاءة LED', 'action');
}
...
window.ledBrightnessChanged = ledBrightnessChanged;
window.ledEffectChanged = ledEffectChanged;
window.setLedPreset = setLedPreset;
window.setPlayerLed = setPlayerLed;
window.applyLedSettings = applyLedSettings;

window.toggleGyroTest = toggleGyroTest;
